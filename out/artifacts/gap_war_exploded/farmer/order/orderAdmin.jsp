      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <div class="row placeholders">
            ...
            <a href="${pageContext.request.contextPath }/farmer/inputProduct.jsp">
          ...
          <div class="table-responsive">
            <table class="table table-striped">
              ...
              <tbody>
                <c:forEach var="order" items="${sessionScope.orderpageBean.list }">
                	
	                 	<tr>
	                		<td><c:out value="${order.id_order }"></c:out></td>
	                		<td><c:out value="${order.productid }"></c:out></td>
	                		<td><c:out value="${order.buyerid }"></c:out></td>
	                		<td><c:out value="${order.buyerzl }"></c:out></td>
	                		<td><c:out value="${order.buynumber }"></c:out></td>

	                		
	                		<td><a type="button" value="确认订单" class="btn btn-sm btn-default"
	                		href="${pageContext.request.contextPath }/OrderAction_NHGetOneOrder?id_order=${order.id_order }">确认订单</a></td>
            ...
            <div style="float: left;">
	            <form method=post id="pageForm" name="customerForm" action="${pageContext.request.contextPath }/ProductAction_NHGetRepository?idOwner_rep=${farmer1.id_nh}">
	 				<!-- 隐藏域.当前页码 -->
					<input type="hidden" name="currentPage" id="currentPageInput" value="${orderpageBean.currentPage }" />
					<!-- 隐藏域.每页显示条数 -->
						...
						<td>
							<span id=pagelink>
								<div style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right">							
									第[<c:out value="${orderpageBean.currentPage }"></c:out>]页,
									共[<c:out value="${orderpageBean.totalPage }"></c:out>]页													
									[<a href="javaScript:void(0)" onclick="changePage(${farmerRepositoryList.currentPage-1 })" style="color:blue" >前一页</A>]
									[<a href="javaScript:void(0)" onclick="changePage(${farmerRepositoryList.currentPage+1 })"  style="color:blue" >后一页</A>] 
	<%-- 								，跳转到
